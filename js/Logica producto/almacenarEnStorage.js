export function almacenarEnStorage(carrito, producto, contadorElementosCarrito, inputCantidad, subTotal, subTotalCarrito) {
    ...
    (localStorage.getItem('subTotalProductos') === null)
        ? subTotal += parseInt(producto.precioProducto * inputCantidad.value)
        : subTotal = parseInt(producto.precioProducto * inputCantidad.value) + Number(localStorage.getItem('subTotalProductos'));
    ...
    console.log(`TEXT CONTEXT DE INPUT CANTIDAD: ${String(inputCantidad.nodeValue)}`);
}